...
export default {
  ...
  methods: {
    ...
    onShortcutTouchStart(element) {
      ...
      this.touch.ancharIndex = ancharIndex
      this.touch.y1 = firstTouch.pageY
      this._scrollTo(ancharIndex)
    },
    onShortcutTouchMove(element) {
      let firstTouch = element.touches[0]
      this.touch.y2 = firstTouch.pageY
      let delta = (this.touch.y2 - this.touch.y1) / ANCHOR_HEIGHT | 0
      let ancharIndex = parseInt(this.touch.ancharIndex) + delta
      this._scrollTo(ancharIndex)
    },
    _scrollTo(index) {
      if (!index && index !== 0) {
      ...
      this.$refs.listview.scrollToElement(this.$refs.listGroup[index], 0) // 第二个参数0位置是滚动动画控制
    },
    _calculateHeight() {
      this.listHeight = []
...
.listview
    ...
    .list-shortcut
      ...
      border-radius: 10px
      text-align: center
      background: $color-list-shortcut
